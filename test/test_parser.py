...
def test_object_properties():
    ...
    expected = [
        ...
        ObjectProperty(
            ObjectProperty(Variable("$object"), Variable("$variable")), "schmariable"
        ),
        ObjectProperty(
            ObjectProperty(Variable("$object"), Variable("$variable")),
...
def test_string_offset_lookups():
    ...
    expected = [
        ...
        BinaryOp(
            ".", ObjectProperty(Variable("$adjacent"), "object"), Variable("$lookup")
        ),
...
def test_string_curly_dollar_expressions():
    ...
    expected = [
        ...
        ObjectProperty(
            ArrayOffset(ObjectProperty(Variable("$object"), "items"), 4), "five"
        ),
            ...
            [
                BinaryOp(
                    ...
                    BinaryOp(
                        ...
                        BinaryOp(
                            ".",
                            BinaryOp(".", 'This is a "', Variable("$heredoc")),
                            '" with some ',
                        ),
                        ObjectProperty(Variable("$embedded"), "variables"),
                    ),
                    ".\nThis is not the EOT; this is:",
                )
            ]
...
def test_method_calls():
    ...
    expected = [
        ...
        MethodCall(
            MethodCall(Variable("$chain"), "one", [Parameter(Variable("$x"), False)]),
            "two",
            [Parameter(Variable("$y"), True)],
        ),
    ]
    eq_ast(input, expected)
...
def test_if():
    ...
    ?>"""
    expected = [
        If(1, If(2, Echo([3]), [], Else(Echo([4]))), [], Else(Echo([5]))),
        If(
            BinaryOp("<", Variable("$a"), Variable("$b")),
            Block([Return(UnaryOp("-", 1))]),
            [
                ElseIf(
                    BinaryOp(">", Variable("$a"), Variable("$b")), Block([Return(1)])
                ),
                ElseIf(
                    BinaryOp("==", Variable("$a"), Variable("$b")), Block([Return(0)])
                ),
            ],
        ...
        If(
            Variable("$if"),
            Block([Echo(["a"])]),
            [ElseIf(Variable("$elseif"), Block([Echo(["b"])]))],
            Else(Block([Echo(["c"])])),
        ),
    ]
    eq_ast(input, expected)
...
def test_foreach():
    ...
    expected = [
        Foreach(
            Variable("$foo"),
            None,
            ForeachVariable(Variable("$bar"), False),
        ...
        Foreach(
            FunctionCall("complex", [Parameter(Variable("$expression"), False)]),
            None,
            ForeachVariable(Variable("$ref"), True),
        ...
        Foreach(
            ...
            Block([FunctionCall("yo", []), FunctionCall("yo", [])]),
        ),
        Foreach(
            Variable("$foo"),
            None,
...
def test_foreach_with_lists():
    ...
    expected = [
        Foreach(
            Variable("$foo"),
            None,
            ForeachVariable([Variable("$bar"), Variable("$baz")], False),
            Block([]),
        ),
        Foreach(
            Variable("$foo"),
...
def test_classes():
    ...
    expected = [
        Class(
            ...
            [
                ClassConstants(
                    [ClassConstant("the", "only"), ClassConstant("constant", "is")]
                ),
                ClassConstants([ClassConstant("change", "chump")]),
                ClassVariables(
                    [], [ClassVariable("$iable", 999), ClassVariable("$nein", None)]
                ),
                ClassVariables(["protected", "static"], [ClassVariable("$x", None)]),
                Method(
...
def test_new():
    ...
    expected = [
        New("Foo", []),
        New("Foo", []),
        New("Bar", [Parameter(1, False), Parameter(2, False), Parameter(3, False)]),
...
def test_exceptions():
    ...
    expected = [
        Try(
            [
                Assignment(
                    Variable("$a"), BinaryOp("+", Variable("$b"), Variable("$c")), False
                ),
            ...
            [
                ...
                Catch(
                    ...
                    [
                        Echo(
                            [BinaryOp(".", "Received namespace food: ", Variable("$f"))]
                        )
                    ],
                ),
                Catch(
...
def test_instanceof():
    ...
    expected = [
        If(
            BinaryOp("instanceof", Variable("$foo"), Constant("Bar")),
            Block([Echo(["$foo is a bar"])]),
            [],
            None,
        ),
...
def test_namespaces():
    ...
    expected = [
        Namespace("my\\name", []),
        Namespace("my\\name", [FunctionCall("foo", []), FunctionCall("bar", [])]),
        Namespace(None, [FunctionCall("foo", []), FunctionCall("bar", [])]),
    ]
    eq_ast(input, expected)
...
def test_use_declarations():
    ...
    expected = [
        ...
        UseDeclarations([UseDeclaration("a", None), UseDeclaration("b", None)]),
        UseDeclarations([UseDeclaration("a", "b"), UseDeclaration("\\c\\d\\e", "f")]),
    ]
    eq_ast(input, expected)
...
def test_constant_declarations():
    ...
    expected = [
        ...
        ConstantDeclarations(
            [
                ConstantDeclaration("bar", "baz"),
                ConstantDeclaration("wat", Constant("\\DOO")),
            ]
        ),
        ConstantDeclarations(
            [ConstantDeclaration("ant", Constant("namespace\\level"))]
        ),
...
def test_closures():
    ...
    expected = [
        Assignment(
            Variable("$greet"),
            Closure(
                [FormalParameter("$name", None, False, None)],
                [],
                [
                    FunctionCall(
                        "printf",
        ...
        Assignment(
            ...
            Closure(
                [
                    FormalParameter("$a", None, False, None),
                    FormalParameter("$b", None, True, None),
                ],
                [LexicalVariable("$c", False), LexicalVariable("$d", True)],
                [],
                True,
            ),
            False,
        ...
        Class(
            ...
            [
                Method(
                    ...
                    [
                        FunctionCall(
                            "p", [Parameter(MagicConstant("__LINE__", 10), False)]
                        ),
                        FunctionCall(
                            "p", [Parameter(MagicConstant("__DIR__", "/my/dir"), False)]
                        ),
                        FunctionCall(
                            "p",
                            [
                                Parameter(
                                    MagicConstant("__FILE__", "/my/dir/file.php"), False
                                )
                            ],
                        ),
                        FunctionCall(
                            ...
                            [
                                Parameter(
                                    MagicConstant("__NAMESPACE__", "Shmamespace"), False
                                )
                            ],
                        ),
                        FunctionCall(
                            ...
                            [
                                Parameter(
                                    MagicConstant("__CLASS__", "Shmamespace\\Bar"),
                                    False,
                                )
                            ],
                        ),
                        FunctionCall(
                            ...
                            [
                                Parameter(
                                    MagicConstant(
                                        "__METHOD__", "Shmamespace\\Bar::__construct"
                                    ),
                                    False,
                                )
                            ],
...
def test_static_scalar_class_constants():
    ...
    expected = [
        Class(
            ...
            [
                ClassVariables(
                    ["public"], [ClassVariable("$b", StaticProperty("self", "C"))]
                ),
                Method(
                    "d",
                    [],
                    [
                        FormalParameter(
                            "$var1", StaticProperty("self", "C"), False, None
                        )
                    ],
                    [],
                    False,
...
def test_backtick_shell_exec():
    ...
    expected = [
        ...
        FunctionCall(
            "shell_exec", [Parameter(BinaryOp(".", "echo ", Variable("$line")), False)]
        ),
    ]
    eq_ast(input, expected)
...
def test_open_close_tags_ignore():
    ...
    expected = [If(1, Block([If(2, 3, [], None), None]), [], Else(Block([0])))]
    eq_ast(input, expected)
...
text
HERE;"""
    expected = [ConstantDeclarations([ConstantDeclaration("X", "text")])]
    eq_ast(input, expected)
...
def test_trait_renames():
    ...
    expected = [
        ...
        Class(
            "B", None, None, [], [TraitUse("T", [TraitModifier("X", "Y", None)])], []
        ),
        Trait("C", [TraitUse("T", [TraitModifier("X", "Y", "public")])], []),
        Trait("D", [TraitUse("T", [TraitModifier("X", None, "public")])], []),
        Trait(
            "E",
            [TraitUse("T", [TraitModifier(StaticProperty("X", "m"), "Y", None)])],
            [],
        ),
    ]
    eq_ast(input, expected)
...
def test_static_expressions():
    ...
    expected = [
        ...
        ConstantDeclarations(
            [ConstantDeclaration("C", BinaryOp("+", 1, BinaryOp("+", 2, 3)))]
        ),
...
def test_const_arrays():
    ...
    expected = [
        ConstantDeclarations(
            [
                ConstantDeclaration(
                    "C", Array([ArrayElement(None, BinaryOp("+", 1, 2), False)])
                )
            ]
        ),
    ]
...
EOT;
    ?>"""
    expected = [Echo(["disregard $all {$crazy} ${stuff}->f();\nand `this`"])]
    eq_ast(input, expected)
